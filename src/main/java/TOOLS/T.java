...
/**
 * An abstract class to gather tools flying around: - rounding double - file
 * naming - ...
 *
...
public abstract class T {
    ...
    /**
     * Returns a String representation of double keeping only some digits after
     * the dot.
     *
    ...
    /**
     * String value for LDraw application directory. Read from the
     * Config.properties ressource bundle.
     */
    public static String LdrawDir_;
    /**
     * String value for the directory where VRML files are produced. Read from
     * the Config.properties ressource bundle.
     */
    public static String WrlOutputDir_;
    /**
     * String value for the directory where VRML data files are stored. This
     * includes color specs, native VRML code replacing common parts (i.e.
     * studs...) Read from the Config.properties ressource bundle.
     */
    public static String WrlDataDir_;
    /**
     * String value for the directory where Ldraw files are produced. Read from
     * the Config.properties ressource bundle.
     */
    public static String LdrawOutputDir_;

    /**
     * Returns the VRML file name corresponding to a .dat LDraw file. In the
     * VRML file name, one keeps the parent directory (either 'p', 'parts' or
     * 'models'. This way, VRML files are stored the same manner as LDraw files.
     *
    ...
    /**
     * Returns the Ldraw output file name corresponding to a .dat LDraw file. In
     * the Ldraw file name, one keeps the parent directory (either 'p', 'parts'
     * or 'models'. This way, new Ldraw files are stored the same manner as
     * original ones.
     *
     * @param filename the LDraw file name
    ...
    /**
     * Returns a new String where 'from' char is replaced by 'to' char.
     *
    ...
    /**
     * Returns a PROTO node used for LDraw sub-parts. This PROTO is used for
     * .dat files that appear in a '1' line.
     *
     ...
     * @param filename the file name where the nodes' code should be found
     * @return a VRML2String
     */
    ...
    /**
     * Returns a VRML2String code from the VRML97 code found in a file. WARNING:
     * side-effect on the VRML2String argument.
     *
    ...
    /**
     * Return an absolute LDraw file name including 'p', 'parts', or 'models'
     * directory. If this file does not exists, returns its argument. File
     * extension must be correct (i.e. .dat).
     *
     * @param filename the name of the LDraw file
     * @return the full LDraw file name, or the argument itself if the full name
     * has not been found.
     */
    ...
    /**
     * Return an directory name (p, parts, model) where the LDraw file is. If
     * this file does not exists, returns 'null'
     *
     * @param filename the name of the LDraw file
     * @return the full LDraw file name, or the argument itself if the full name
     * has not been found.
    ...
    /**
     * Return a complete VRML data file name including the correct path prefix
     * and 'p', 'parts', or 'models' directory. File extension is ignored and
     * replaced by '.wrl'.
     *
     * @param filename the name of the VRML data file
     * @return the full LDraw file name, or the <code>null</code> if the file
    ...
    /**
     * Builds and returns a PROTO from a Java class. The Java class is
     * dynamiclly loaded and it replaces the LDraw sub-part of the same name.
     *
     * @param name the name of the returned PROTO
     * @param partName the name of the LDraw sub-part
     * @param remaining the transformation matrix to be applied to the sub-part
     * @return a PROTO node
     */
    ...
    /**
     * Finds and returns a file name searched in the 'dirs' array. If the can
     * not be found returns 'null'.
     *
    ...
    /**
     * Finds a file name searched in the 'dirs' array. If it can not be found
     * returns 'null', otherwise retruns the directory name
     *
     * @param filename the file base name
     * @param dir the base directory
