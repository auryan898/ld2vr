...
/**
 * This class is the common root for all VRML nodes that can be DEFed. That's
 * all nodes but the PROTO. This class offers very simple DEF/USE mechanism that
 * is totally under the programmer control: nothing here will DEF a node
 * automagically if the programer did not request it previously.
 * <em>Moreover, if an already DEFed node is modified, these MODIFICATIONS will
 * be LOST!</em>
 */
class NamedNode extends VRML2Node {

    /**
     * Builds an unnamed node.
     */
    NamedNode() {
        name_ = null;
        namep_ = false;
    ...
    /**
     * Builds a named node. The name is generated using a constant prefix then
     * an index, and a constant suffix.
     *
     * @param namep a boolean indicating if the node should be named or not
     */
    ...
    /**
     * Returns a String representation of a named node. If the node is unnamed,
     * it is always printed as is. If the node is named and has not been printed
     * yet, return the String representation of the node preceded by "DEF
     * <name>". If the node has been printed simply returns "USE <name>".
     *
    ...
    /**
     * Returns a unique name made with a constant prefix then an index, and a
     * constant suffix.
     *
